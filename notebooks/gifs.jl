...
# ╔═╡ 1c87d1e2-82d9-11ec-08ee-a184d2bf4917
using Reel

# ╔═╡ 6593256c-e0a3-4740-9b69-6325bed37cc5
using Images

# ╔═╡ f2d0a9b7-2832-46c6-bd18-933790770686
using PlutoUI

# ╔═╡ c2362e14-68f5-4ff1-b1d5-83d654f6160d
function create_planning_gif(directory, max_steps; 
		input=t->"b$t.png", output="output.gif", freeze_last=true, last=10, fps=1)
	frames = Frames(MIME("image/png"), fps=fps)

	local frame
	for t in 0:max_steps
		frame = load(joinpath(directory, input(t)))
		push!(frames, frame)
	end
	if freeze_last
		[push!(frames, frame) for _ in 1:last] # duplicate last frame
	end
	write(output, frames)
	LocalResource("./$output")
end

# ╔═╡ 3a99b4e5-462b-4abd-ab1d-a83f43b84ea3
function create_planning_gif_combined(directory, max_steps; 
		input1=t->"b$t.png", input2=t->"b$(t)_hist.png", output="output.gif", freeze_last=true, last=10, fps=1)
	frames = Frames(MIME("image/png"), fps=fps)

	local frame
	for t in 0:max_steps
		frame_top = load(joinpath(directory, input1(t)))
		frame_bottom = load(joinpath(directory, input2(t)))
		frame = vcat(frame_top, frame_bottom)
		push!(frames, frame)
	end
	if freeze_last
		[push!(frames, frame) for _ in 1:last] # duplicate last frame
...
# ╔═╡ dc956428-7bc1-4b2e-adc5-93456f5786e8
function combine_figures(directory, top_name, bot_name; side_by_side=false, output="combined.png")
	top = load(joinpath(directory, top_name))
	bot = load(joinpath(directory, bot_name))
	if side_by_side
		combined = hcat(top, bot)
	else
...
# ╔═╡ 1d463de4-727d-4083-bae0-5a02db8e090d
md"""
# Gifs
"""

# ╔═╡ a86da4d8-cfe1-4fe1-b099-5c5dd1bb9a2a
directory =
"C:\\Users\\mossr\\Code\\sisl\\CCS\\MineralExploration\\scripts\\data\\demos\\multishape_20x20\\"

# ╔═╡ f9492668-0b51-4e81-897b-168492e27873
max_steps = 24

# ╔═╡ c183e84f-7e78-4a6f-9b90-087d1c7b25ef
create_planning_gif(directory, max_steps; input=t->"b$t.png", output="belief.gif")

# ╔═╡ fd03d502-432b-4abf-9f34-b71e9aa253c4
create_planning_gif(directory, max_steps; input=t->"b$(t)_hist.png", output="hist.gif")

# ╔═╡ 48b93f40-0898-48fe-b249-ff6ae5d49cac
md"""
---
"""

# ╔═╡ 6883d250-5ef3-4735-86ad-bd69c3e6f326
directory10x10 =
"C:\\Users\\mossr\\Code\\sisl\\CCS\\MineralExploration\\scripts\\data\\demos\\multishape_10x10\\"

# ╔═╡ f217923b-12eb-42ee-8811-a0f095c7a36e
create_planning_gif(directory10x10, 7;
	input=t->"b$t.png", output="belief10x10.gif")

# ╔═╡ 73c2e83e-1a40-4de8-b804-318ac336c047
md"""
# Sandbox
"""
...
# ╔═╡ 242c0769-7fa1-4449-81e2-e1a24b400651
md"""
```julia
run_single_trial(5, 50, 10_000; MainbodyType=BlobNode, save_dir=SAVE_DIR)
# ~900 seconds
```
"""

# ╔═╡ efc9dc67-91d9-4e70-be30-cd3979d66734
combine_figures(directory_sandbox, "ore_map.png", "mass_map.png"; side_by_side=false)

# ╔═╡ b1d40202-92d3-47db-b930-88a58e4096cf
create_planning_gif_combined(directory_sandbox, sandbox_max;
	output="combined_sandbox.gif", fps=2, last=20)

# ╔═╡ 7567b752-9bc7-4e84-84ae-a2eeb9006a66
md"""
# Sandbox 2
"""
...
md"""
```julia
run_single_trial(5, 30, 10_000; MainbodyType=BlobNode, save_dir=SAVE_DIR)
# ~129 seconds
```
"""

# ╔═╡ 9ff7c61a-cd52-42ca-aec1-f36afb1e6fdc
combine_figures(directory_sandbox2, "ore_map.png", "mass_map.png"; side_by_side=false)
